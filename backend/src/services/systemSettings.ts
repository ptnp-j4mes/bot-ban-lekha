...
export async function getSystemSettings() {
  ...
  cache = { defaultBillFooter: row.defaultBillFooter, defaultTimezone: row.defaultTimezone, defaultSlipRetentionDays: row.defaultSlipRetentionDays };
  return cache;
}

export async function updateSystemSettings(data: { defaultBillFooter?: string | null; defaultTimezone?: string; defaultSlipRetentionDays?: number | null }) {
  const row = await prisma.systemSetting.upsert({
    where: { id: "system" },
    update: data,
