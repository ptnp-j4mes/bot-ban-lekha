...
export function CustomerHomeScreen() {
  ...
  const { displayName, customerCode, oaId, logout } = useCustomerAuth();
  const queryScope = oaId || customerCode ? [oaId ?? null, customerCode ?? null] as const : [] as const;
  const balance = useQuery({ queryKey: ['customer-balance', ...queryScope], queryFn: () => api.get<CustomerBalance>('/api/liff/me/balance', { org: false }) });
  const installments = useQuery({ queryKey: ['customer-installments', ...queryScope], queryFn: () => api.get<CustomerInstallment[]>('/api/liff/me/installments', { org: false }) });
