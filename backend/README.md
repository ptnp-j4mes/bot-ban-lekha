...
- Storage: switchable between local disk, Google Drive, and AWS S3/S3-compatible storage. `local` needs no external config; `gdrive` and `s3` are rejected until their required config is complete.
