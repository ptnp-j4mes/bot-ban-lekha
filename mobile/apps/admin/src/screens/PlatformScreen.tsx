...
export function PlatformScreen() {
  ...
  const { enterOrg, me, orgId } = useAdminAuth();
  const queryScope = me?.user_id || orgId ? [me?.user_id ?? null, orgId ?? null] as const : [] as const;
  const organizations = useQuery({ queryKey: ['mobile-platform-organizations', ...queryScope], queryFn: () => api.get<any>('/api/platform/organizations', { org: false }) });
