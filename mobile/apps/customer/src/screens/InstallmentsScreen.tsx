...
export function InstallmentsScreen() {
  ...
  const { customerCode, oaId } = useCustomerAuth();
  const queryScope = oaId || customerCode ? [oaId ?? null, customerCode ?? null] as const : [] as const;
  const list = useQuery({ queryKey: ['customer-installments', ...queryScope], queryFn: () => api.get<CustomerInstallment[]>('/api/liff/me/installments', { org: false }) });
